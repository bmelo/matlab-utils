...
classdef Dcm2nii < handle
    ...
    properties(Constant)
        exePath = fullfile(fileparts(mfilename('fullpath')), '..\vendors', 'dcm2nii.exe');
        iniPath = fullfile(fileparts(mfilename('fullpath')), '..\vendors', 'dcm2nii.ini');
    end
    ...
    methods(Access = private)
        ...
        function cmd=montaComando(obj, img)
            ...
            cmd = [exe out gzip ini ' "' fullfile(obj.dirImgCur, img) '"'];
        end
        ...
        function percorrerSubDirs(obj, raiz)
            ...
            for k=1:length(subDirs)
                ...
                if isempty( find(strcmp(subDirs(k).name, {'.','..'}) == 1) )
                    switch exist(itempath)
                        ...
                        otherwise
                            ...
                            obj.dirImgCur = raiz;
                            obj.converter(subDirs(k).name); %Faz a conversão
