...
for nses=1:obj.numSessions
  ...
  for ncond=1:numCond
    ...
    %Gerando informações para as modulações paramétricas
    if isstruct(obj.paramModulation)
      ...
      try
        ...
        if( length(params.(paramsNames{1}).vals) == totalOnsets ) %Só insere se a quantidade de onsets for igual
