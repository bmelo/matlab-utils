...
if regexp(hname, '\.gz$') > 0
    hdr = javaMethod('read', 'niftijio.NiftiHeader', hname);
    hdr = utils.java.convert_object(hdr);
    
    if isempty( hdr.dim_info )
       hdr.dim_info = 0;
    end
    
    hdr.quatern_b = hdr.quatern(1);
    hdr.quatern_c = hdr.quatern(2);
    hdr.quatern_d = hdr.quatern(3);
    
    hdr.qoffset_x = hdr.qoffset(1);
    hdr.qoffset_y = hdr.qoffset(2);
    hdr.qoffset_z = hdr.qoffset(3);
    
    be = ~hdr.little_endian;
    return;
end
