...
for s=1:length(subjs{1})
    ...
    matlabbatch{1}.spm.stats.factorial_design.des.t1.scans1{s,1} = fullfile(dir_base, 'STATS\FIRST_LEVEL', design, subj, contr);
end
for s=1:length(subjs{2})
    subj = sprintf( 'SUBJ%03i', subjs{2}(s) );
    matlabbatch{1}.spm.stats.factorial_design.des.t1.scans2{s,1} = fullfile(dir_base, 'STATS\FIRST_LEVEL', design, subj, contr);
end
...
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = sprrintf('%s_%s > %s', contrast{k} ,groupname{1}, groupname{2});
matlabbatch{3}.spm.stats.con.consess{1}.tcon.convec = [1 -1];
matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
matlabbatch{3}.spm.stats.con.consess{2}.tcon.name = sprrintf('%s_%s > %s', contrast{k} ,groupname{2}, groupname{1});
