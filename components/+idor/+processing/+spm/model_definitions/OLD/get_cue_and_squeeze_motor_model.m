...
% pres_codes
function model = get_cue_and_squeeze_motor_model()
    ...
    %% contrastes entre condicoes
    ci = ci + 1;
    model.contrast(ci).vec  = [ 1 -1 0 -1];
    model.contrast(ci).name = 'CS FLA > CS STI (SUB EFFORT)';
    
    ci= ci+1;
    
    model.contrast(ci).vec  = [ -1 1 0 -1];
    model.contrast(ci).name = 'CS STI > CS FLA (SUB EFFORT)';
    
    ci= ci+1;
       
    model.contrast(ci).vec  = [ 1 0 -1 -1];
    model.contrast(ci).name = 'CS FLA > CS SELF (SUB EFFORT)';

    ci= ci+1;
       
    model.contrast(ci).vec  = [ -1 0 1 -1];
    model.contrast(ci).name = 'CS SELF > CS FLA (SUB EFFORT)';

    ci = ci+1;
    model.contrast(ci).vec  = [ 0 1 -1 -1];
    model.contrast(ci).name = 'CS STI > CS SELF (SUB EFFORT)';
    
    ci= ci+1;
    
    model.contrast(ci).vec  = [ 0 -1 1 -1];
    model.contrast(ci).name = 'CS SELF > CS STI (SUB EFFORT)';
