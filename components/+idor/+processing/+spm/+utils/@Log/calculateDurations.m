...
if hasTermination
    type_matches = obj.get_matches(  def.pres_termination_types, obj.EventType );
    
    % find all code matches
    code_matches = obj.get_matches( def.pres_termination_codes, obj.Code );
    ...
    %Irá remover itens que não sejam do tipo determinado
    if filterValue 
    ...
    for k = 1:length(idx_on)
        ...
        if isempty( idx_diff )
            ...
            isOk = ~filterValue || any( idx_ok == m_end ); %Avisa que se encaixa e que irá usar este onset
            if( ~isOk )
                durations(k) = -1; %Flag para remoção de onset
            elseif( hasFix )
                durations(k) = def.spm_fix_duration; %Usa tempo fixo
            else
                durations(k) = obj.timereal( m_end ) - obj.timereal( m ); %Calcula duração
            end
        end
    end
else
