...
for i=subjs
    ...
    for vis = 1:length(sdirs)
        ...
        if norm_anat
            ...
            fprintf( 'Normalization structural images for subject: %s\n%s\n', name_subj{i}, preproc_dir );
            spm_jobman('run',matlabbatch);
        else
            %%%%%%% Normalization %%%%%%%%%%%%
            clear matlabbatch;
            normalize_affine
            save( fullfile( preproc_dir, 'BATCH_3_NORM_AFFINE.mat'), 'matlabbatch' );
            fprintf( 'Normalization structural images for subject: %s\n%s\n', name_subj{i}, preproc_dir );
            spm_jobman('run',matlabbatch);
        end
        ...
        fprintf( 'Smoothing functional images for subject: %s\n%s\n', name_subj{i}, preproc_dir);
        spm_jobman('run',matlabbatch);
