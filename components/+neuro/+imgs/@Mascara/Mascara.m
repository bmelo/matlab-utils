classdef Mascara < Generic
    ...
    properties(SetAccess = private)
        atlas;
        img; %Ir� armazenar a imagem da m�scara
        id = '';
    end
    ...
    methods
        ...
        %Preenche a mascara com todas as �reas do atlas
        function fill( obj )            
        ...
        %Verifica se uma area j� faz parte da mascara
        function existe = hasArea(obj, area)
            areas = obj.listValues();
            existe = any( areas==area );
        end
        
        %faz a convers�o para o espaco do volume passado como par�metro
        function converterBaseSubj(obj, volBase, output)
            %verifica��es
            if( isempty( fileparts(volBase) ) ) %Caso s� exista o nome do arquivo, coloca o diret�rio atual
            ...
            cd(output);
            tmpVolBase = [tempname(output) '.nii']; %gera nome tempor�rio
            copyfile( volBase, tmpVolBase );
            mask = fullfile(output, 'mask.nii'); %gera nome tempor�rio
        ...
        %Exibe a lista das �reas presentes na mascara
    ...
    methods(Access = private)   
        ...
        %Copia os arquivos .mat para acelerar a convers�o do espa�o da mask
        function copiarMats(obj, origem, destino, nameVol, toConvert )          
            ...
            for k=1:length(files) %Percorre os arquivos e copia o que for necess�rio
        ...
        function gerarId(obj)
            %Monta um identificador �nico para a mascara
