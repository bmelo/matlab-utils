function varargout = art_IDOR(varargin)
...
mvmt_diff_thresh = 2.0;     %scan-to-scan subject motion threshold
...
datafiles=cell(1,length(P));for i=1:length(P),if ~isempty(P{i}),datafiles{i}=strtrim(P{i}(1,:));else datafiles{i}=[];end;end; % <alfnie>: keep filenames of functional data (first scan per session only)
...
if drop_flag, disp('warning: explicitly dropping 1st scan no longer supported. Edit the ''all outliers'' box to specify any number of additional outlier scans'); end % dropflag no longer supported: (alfnie 06/11)
...
%in FSL order of fields is three Euler angles (x,y,z in radians) then three translation params (x,y,z in mm).
...
diff_similiarity = cell(1,num_sess); % g is a cell array of the global mean for each scan in each session
...
for sess=1:num_sess
    ...
    % --------------------------------------------------
    % Extracts global-signal mask and (optinally) compute analysis mask
    ...
    for i = 1:nscans,
        ...
        if badmask, g{sess}(i) = spm_global(VY{sess}(i));
            ...
            % Sebastian
            if i>1
                diff_similiarity{sess}(i-1) = corr( temp(idxMask)', temp_before' );
            end
            temp_before = temp(idxMask);
        end
...
% 33-51 : scan-to-scan differences in 6 control points trajectories
% 52    : composite measure: max scan-to-scan movement across the 6 control points
...
if use_diff_global||use_diff_motion
    diffsglobalandmotion_Callback(hObject, [], handles, use_diff_global, use_diff_motion);
else
...
if 1 %(get(handles.showCorr,'Value') == get(handles.showCorr,'Max'))
    ...
    for sess=1:length(sessions),
        ...
        cm_tmp{sess} = cm{sess}(1:end-6,end-5:end);
        colorbar;
        xlabel_names{sess} = {'x','y','z','pitch','roll','yaw'};
        ylabel_names{sess} = names;
    ...
    set( f, 'units', 'normalized', 'outerposition', [ 0 0 1 1], 'paperpositionmode', 'auto' );
    output_dir = getappdata(handles.savefile,'dir');
    saveas( f, fullfile( output_dir, 'motion_task_correlation.fig') );
    saveas( f, fullfile( output_dir, 'motion_task_correlation.png') );
    cm = cm_tmp;
    save( fullfile( output_dir, 'motion_task_correlation.mat'), 'cm', 'xlabel_names', 'ylabel_names');
else
    f = getappdata(handles.showCorr,'figure');
    if ishandle(f)
        close(f);
...
if 1 %(get(handles.sigCorr,'Value') == get(handles.sigCorr,'Max'))
    %display correlations
    ...
    for sess=1:length(sessions)
        ...
        cm_tmp{sess} = cm{sess}(end:end,1:end-1);
        colorbar;
        names=cat(2,SPM.Sess(s).U(1:length(cols)).name);
        xlabel_names{sess} = names;
        ylabel_names{sess} = 'mean_activation';
    ...
    set( f,'units','normalized','outerposition',[0 0 1 1],'PaperPositionMode', 'auto' )
    output_dir = getappdata(handles.savefile,'dir');
    saveas( f, fullfile( output_dir, 'global_signal_task_correlation.fig') );
    saveas( f, fullfile( output_dir, 'global_signal_task_correlation.png') );
    cm = cm_tmp;
    
    save( fullfile( output_dir, 'global_signal_task_correlation.mat'), 'cm', 'xlabel_names', 'ylabel_names' );
else
    f = getappdata(handles.sigCorr,'figure');
    if ishandle(f)
        close(f);
...
if (get(handles.showSpec,'Value') == get(handles.showSpec,'Max'))
    ...
    output_dir = getappdata(handles.savefile,'dir');
    saveas( gcf, fullfile( output_dir, 'task_spectra.fig') );
    saveas( gcf, fullfile( output_dir, 'task_spectra.png') );
...
for s=S(:)',
    switch s
        case 1 %save outliers
            ...
            output_dir = getappdata(handles.savefile,'dir');
            save( fullfile( output_dir, 'outliers.txt'), 'out_idx', '-ascii' );
        ...
        case 2 %save motion statistics
            ...
            save(fullfile( output_dir, 'mov_stats.mat'),'mv_stats','analyses','-mat');
        ...
        case 3 %save graphs
            ...
            saveas(gcf,fullfile( output_dir, 'screen_withdesign.fig'));
            saveas(gcf,fullfile( output_dir, 'screen_withdesign.png'));
            
           
            set( handles.showDesign,'Value', 0);
            UpdateSummaryplot( hObject, eventdata, handles);
            
            saveas(gcf,fullfile( output_dir, 'screen.fig'));
            saveas(gcf,fullfile( output_dir, 'screen.png'));
...
mvthresh = get(handles.mvthresh,'String');
zthresh = get(handles.zthresh,'String');
...
for j=1:num_sess,
    ...
    try
        R=R1;save(fullfile(datafiles_path,['art_regression_outliers_',datafiles_name,'_', mvthresh, '_', zthresh,'.mat']),'R','-mat'); %#ok<NASGU>
        R=R2;save(fullfile(datafiles_path,['art_regression_outliers_and_movement_',datafiles_name,'_', mvthresh, '_', zthresh,'.mat']),'R','-mat'); %#ok<NASGU>
    catch
        try
