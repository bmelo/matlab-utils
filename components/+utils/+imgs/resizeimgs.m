function resizeimgs(directory, wh, isrecursive, isoverwrite, savetopath, supportFormat, quality)
% resizeimgs: resize a batch of photos
%
% resizeimgs -dir max_width_and_height, will let you choose a directory, 
...
% 
% resizeimgs -files max_width_and_height, will let you choose some files,
...
% 
% Example:
%           resizeimgs('.', [1920, 1920], true, false, 'output\', '*', 85)
%
...
% 
% resizeimgs(file_or_directory, max_width_and_height, is_recursive, ...
...
% author: zhang@zhiqiang.org
% adapted by: Bruno Melo <bruno.raphael@gmail.com>
...
if ~exist('quality', 'var'), quality = 88; end
if nargin <= 5 || isempty(supportFormat), supportFormat = '*'; end
if isempty(directory); directory = uigetdir; end;
...
if iscell(directory)
    for i = 1:numel(directory)
        resizeimgs(directory{i}, wh, isrecursive, isoverwrite, savetopath, supportFormat, quality);
    end
    return;
end
...
if exist(directory, 'file') &&  ~isdir(directory) % for a file, 
    ...
    resizesinglephoto(directory, wh, savetopath, isoverwrite, quality);
    ...
    for i = 1:numel(allfiles)
        ...
        if numel(cur) > 2 || ~min(cur == '.')
            if allfiles(i).isdir && isrecursive
                resizeimgs([directory cur '\'], wh, isrecursive, isoverwrite, ...
                    [savetopath cur '\'], supportFormat, quality);
            elseif ~allfiles(i).isdir
                resizesinglephoto([directory cur], wh, savetopath, isoverwrite, quality);
            end
        end
    end 
end
...
function resizesinglephoto(photo, wh, savetopath, isoverwrite, quality)
...
I.resize(wh);
if ~isoverwrite
    %I.renameWithTime(true);
end
I.save(savetopath);
disp([photo ' is resized, and is saved to ' savetopath I.name I.ext]);
