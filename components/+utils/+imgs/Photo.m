classdef Photo < handle
    ...
    methods
        function obj = Photo( photoPath )
            obj.fullPath = photoPath;
            [obj.path obj.name obj.ext] = fileparts(photoPath);
            obj.ext = lower(obj.ext);
            if strcmp(obj.ext, '.jpeg'); obj.ext = '.jpg'; end;
            obj.dataImg = imread(photoPath);
            obj.info = imfinfo(photoPath);
            try
